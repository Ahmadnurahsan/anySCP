...
export type PageId = "hosts" | "snippets" | "port-forwarding" | "history" | "settings" | "transfers" | "plugins";
...
export type UnifiedTab =
  ...
  | { type: "tools"; id: string; label: string }
...
interface TabState {
  ...
  activateRecentTabOfType: (type: "terminal" | "sftp" | "s3" | "tools") => boolean;
}
...
function syncDomainStores(tab: UnifiedTab) {
  if (tab.type === "terminal") {
    ...
    void import("./tools-store").then(({ useToolsStore }) =>
      useToolsStore.getState().setActiveToolsSession(tab.id),
    );
  }
}
